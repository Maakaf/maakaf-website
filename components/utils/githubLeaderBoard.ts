...
export function calculateTotals(weeks: GithubContributorStats[number]['weeks']) {
    return weeks.reduce((acc, week) => {
        ...
        return {
            score: acc.score + (stats.additions * 3 + stats.deletions * 2 + stats.commits),
            stats
        };
  ...
  .reduce((acc, data) => {
      ...
      .map(c => {
        ...
        return {
            ...
            projects_names: {url: `${data.value.owner}/${data.value.repo}`, name: data.value.repo},
      ...
      for (const member of members) {
        if (!acc.has(member.node_id)) {
            acc.set(member.node_id, {...member, projects_names: [member.projects_names]});
        } else {
